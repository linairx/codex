...
usage() {
  cat <<'EOF'
Usage: scripts/capture-gateway-promotion-baseline.sh --bundle DIR --gateway-url URL [options]
       just gateway-promotion-baseline-capture -- --bundle DIR --gateway-url URL [options]

Options:
  --prefix VALUE          Artifact filename prefix. Default: 01-baseline.
  --scenario VALUE        Evidence scenario row to update. Default: Baseline before traffic.
  ...
  --captured-by VALUE     Fill README Captured by when it is empty.
  --bearer-token VALUE    Bearer token for the gateway health request.
  --capture-time VALUE    ISO UTC capture timestamp to use in artifacts.
  -h, --help
EOF
}
...
gateway_url=
prefix=01-baseline
scenario="Baseline before traffic"
tenant_id=
project_id=
worker_build=
worker_id=
...
for required in README.md worksheet.md transcripts healthz events metrics logs; do
...
write_artifact "$bundle/transcripts/$prefix.txt" printf '%s\n' "baseline health capture from $health_url"
write_artifact "$bundle/healthz/$prefix.json" cat "$health_body"
write_artifact "$bundle/events/$prefix.sse" printf '%s\n' "baseline capture: start /v1/events before traffic scenarios that need operator events"
write_artifact "$bundle/metrics/$prefix.json" printf '%s\n' '{"note":"export gateway metrics for this capture window from the configured telemetry backend"}'
write_artifact "$bundle/logs/$prefix.log" printf '%s\n' "baseline capture: export gateway audit and structured logs for this capture window"

replace_field_if_empty() {
  path=$1
  prefix_value=$2
  replacement=$3
  tmp=$(mktemp)
  awk -v prefix="$prefix_value" -v replacement="$replacement" '
    index($0, prefix) == 1 {
      value = $0
      sub("^" prefix, "", value)
      gsub(/^ +| +$/, "", value)
...
replace_field_if_empty "$bundle/README.md" '- Capture start:' "$capture_time"
replace_field_if_empty "$bundle/README.md" '- Capture end:' "$capture_time"
if [ -n "$captured_by" ]; then
  replace_field_if_empty "$bundle/README.md" '- Captured by:' "$captured_by"
fi
...
replace_scenario_row \
  "$bundle/worksheet.md" \
  "| $scenario | transcripts/$prefix.txt | healthz/$prefix.json | events/$prefix.sse | metrics/$prefix.json | logs/$prefix.log | captured |"
