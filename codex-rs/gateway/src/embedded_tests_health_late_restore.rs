...
macro_rules! legacy_account_handoff_test_setup {
    () => {{
        ...
        (worker_a, worker_b, server, client)
    }};
}
...
async fn remote_multi_worker_restores_thread_read_after_account_exhaustion_over_v2() {
    ...
    assert_eq!(restored.thread.id, worker_b_thread_id);
    assert_eq!(
...
async fn remote_multi_worker_restores_thread_unarchive_after_account_exhaustion_over_v2() {
    ...
    assert_eq!(unarchive_response.thread.id, worker_b_thread_id);
    assert_eq!(
...
#[path = "embedded_tests_health_late_restore_metadata.rs"]
mod embedded_tests_health_late_restore_metadata;
