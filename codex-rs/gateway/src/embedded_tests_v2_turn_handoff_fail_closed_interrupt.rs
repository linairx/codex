...
#[tokio::test]
async fn remote_multi_worker_fails_closed_when_turn_interrupt_has_no_account_replacement_over_v2() {
            ...
            loop {
                ...
                while let Some(event_end) = buffered.find("\n\n") {
                    ...
                    if event.contains("event: gateway/accountActiveThreadHandoffFailed")
                        && event.contains("\"method\":\"turn/interrupt\"")
                    {
                        return event;
                    }
                }
    ...
    assert!(
        request_error
            ...
            .contains(
                format!(
                    "thread {thread_id} is pinned to worker 1 with exhausted account capacity for turn/interrupt"
                )
                .as_str()
            ),
        "{request_error}"
    ...
    assert!(handoff_event.contains("\"method\":\"turn/interrupt\""));
