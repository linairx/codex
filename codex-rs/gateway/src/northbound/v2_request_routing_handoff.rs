//! Account-handoff and visible-thread restoration helpers for northbound v2 client handling.
//!
//! This module owns the thread-id restoration paths that may retry on another
//! account-backed worker when the cached worker is exhausted, so the remaining
//! request-routing module can focus on fanout and connection-scoped routing.
...
pub(crate) async fn recover_visible_thread_worker_route(
    ...
    for worker in &downstream.workers {
        let response = worker
            .request_handle
            .request(codex_app_server_protocol::ClientRequest::ThreadRead {
                request_id: codex_app_server_protocol::RequestId::String(format!(
                    "gateway-thread-route-probe:{thread_id}"
                )),
