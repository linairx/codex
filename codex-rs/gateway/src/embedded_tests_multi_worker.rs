            ...
            && saw_turn_completed)
        {
            ...
            match event {
                ...
                AppServerEvent::ServerNotification(ServerNotification::ServerRequestResolved(
                    ...
                    assert_eq!(dynamic_tool_request_id.as_ref(), Some(&request_id));
                ...
                AppServerEvent::ServerNotification(ServerNotification::TurnCompleted(
                    ...
                    assert_eq!(
                        saw_dynamic_tool_item_started, true,
                        "dynamic tool item should start before turn completion"
                    );
                    assert_eq!(
                        saw_dynamic_tool_resolved, true,
                        "serverRequest/resolved should arrive before turn completion"
                    );
                    assert_eq!(
                        saw_dynamic_tool_item_completed, true,
                        "dynamic tool item should complete before turn completion"
                    );
        ...
        loop {
            ...
            match event {
                ...
                AppServerEvent::ServerNotification(ServerNotification::TurnCompleted(
                    ...
                    assert_eq!(
                        saw_user_input_request, true,
                        "user-input request should arrive before turn completion"
                    );
                    assert_eq!(
                        saw_user_input_resolved, true,
                        "user-input serverRequest/resolved should arrive before turn completion"
                    );
                    assert_eq!(
                        saw_command_request, true,
                        "command approval should arrive before turn completion"
                    );
                    assert_eq!(
                        saw_file_request, true,
                        "file approval should arrive before turn completion"
                    );
                    assert_eq!(
                        saw_mcp_elicitation_request, true,
                        "mcp elicitation should arrive before turn completion"
                    );
                    assert_eq!(
                        saw_dynamic_tool_item_started, true,
                        "dynamic tool item should start before turn completion"
                    ...
                    assert_eq!(
                        saw_dynamic_tool_request, true,
                        "dynamic tool request should arrive before turn completion"
                    );
                    assert_eq!(
                        saw_dynamic_tool_resolved, true,
                        "dynamic tool serverRequest/resolved should arrive before turn completion"
                    );
                    assert_eq!(
                        saw_dynamic_tool_item_completed, true,
                        "dynamic tool item should complete before turn completion"
                    ...
                    assert_eq!(
                        saw_permissions_request, true,
                        "permissions approval should arrive before turn completion"
                    );
                    assert_eq!(
                        saw_refresh_request, true,
                        "chatgpt refresh should arrive before turn completion"
                    );
                    saw_turn_completed = true;
                }
                _ => {}
