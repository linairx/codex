...
- `thread/closed`、`thread/archived`、`thread/unarchived` 仍只是 lifecycle
  边事件，不应被 SQLite 收敛层重新理解成完整线程摘要来源
- `thread/name/updated` 也仍只是 name 增量；如果消费者需要在本地视图上同步名字，
  应把它应用到最近一次权威 `Thread` 摘要，而不是把通知本身当成替代读取面
...
同一条边界也应继续覆盖 `thread/closed`、`thread/archived`、
`thread/unarchived`、`thread/name/updated`：这些通知要么只携带 lifecycle edge，
要么只携带 name 增量，都不应被 SQLite 收敛层重新包装成完整线程恢复摘要。
