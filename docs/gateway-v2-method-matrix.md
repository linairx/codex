...
| `thread/resume` | Resume an existing session | `policy interception` | `threadId`-based resume is covered by single-worker remote compatibility tests plus real multi-worker routing and same-session recovery regressions. Gateway scope policy still rejects `history`- or `path`-based resume because those forms bypass thread ownership checks, and that rejection path is now covered by real embedded, single-worker remote, and multi-worker remote `RemoteAppServerClient` compatibility tests in addition to dedicated northbound JSON-RPC regressions. |
| `thread/fork` | Fork an existing session | `policy interception` | `threadId`-based fork is covered by single-worker remote compatibility tests plus real multi-worker routing and same-session recovery regressions, including follow-up `thread/read` route backfill for the returned forked thread. Gateway scope policy still rejects `path`-based fork because it bypasses thread ownership checks, and that rejection path is now covered by real embedded, single-worker remote, and multi-worker remote `RemoteAppServerClient` compatibility tests in addition to dedicated northbound JSON-RPC regressions. |
