...
| `skills/config/write` | Enable or disable a skill by path/name | `transparent passthrough` | Covered by dedicated gateway passthrough tests for the low-frequency skill configuration write path, and now also by the real single-worker remote `RemoteAppServerClient` setup harness. Multi-worker remote mode now fans this setup mutation out across workers and has real shared-session coverage, including same-session recovery coverage after a worker is re-added, so worker-local skill enablement does not drift behind the gateway boundary. |
...
| `marketplace/add` | Add a plugin marketplace source | `transparent passthrough` | Covered by dedicated gateway passthrough tests for the low-frequency marketplace onboarding path, and now also by the real single-worker remote `RemoteAppServerClient` setup harness. Multi-worker remote mode now fans this setup mutation out across workers and has real shared-session coverage, including same-session recovery coverage after a worker is re-added, so worker-local marketplace state stays aligned. |
