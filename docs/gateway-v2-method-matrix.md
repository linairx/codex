...
- transport model: one northbound v2 connection maps to one downstream
  app-server session
- multi-worker remote runtime now has partial Stage B support; hardening is
  complete, this matrix keeps Stage A as the drop-in baseline, and it calls out
  the current multi-worker additions separately where they affect rollout
  guidance and the rollout gate
...
| bounded account handoff | `thread/read`, thread-id `thread/resume` / `thread/fork`, rollout-path `thread/resume` / `thread/fork`, legacy `getConversationSummary` variants, and resumable thread-id controls such as `thread/name/set`, `thread/memoryMode/set`, `thread/rollback`, `thread/archive`, `thread/unarchive`, `thread/metadata/update`, `thread/turns/list`, `thread/increment_elicitation`, `thread/decrement_elicitation`, `thread/inject_items`, `thread/unsubscribe`, and `thread/compact/start` | replacement-account success preserves the requested thread id or rollout path, updates sticky routing, emits the expected `/v1/events` account event, and records matching account-capacity metrics plus `accountCapacityEvent*` health fields; direct regressions now pin that health / events / metrics agreement for representative thread-id, rollout-path, conversation-summary, and additional thread-control success and failure paths, including the no-thread-response `thread/unsubscribe` and `thread/compact/start` success paths, and the real multi-worker `RemoteAppServerClient` harness now also validates the successful replacement-account and fail-closed branches for those no-thread-response controls with follow-up `thread/read` stickiness after success, plus the `conversation_summary_handoff_success` / `conversation_summary_handoff_failure` labels and the path-based `path_thread_handoff_success` / `path_thread_handoff_failure` event labels and metric counts used by rollout-path restoration |
| project-aware account routing | project-scoped `thread/start`, `/healthz.projectWorkerRoutes`, `remoteWorkers[].accountId` | same-project affinity stays pinned to the recorded project route, different projects prefer less-loaded eligible account-backed workers, and the real multi-worker scope harness verifies `remoteAccountLabelsComplete=true` plus the resulting project/worker/account bindings; the remote runtime health snapshot now also pins the same registered project-to-worker map, including `projectWorkerRoutes[].accountRoutingEligible` so the selected route's worker health, account label, and account-capacity state can be reviewed as one field, and `/healthz.v2Connections.projectWorkerRouteSelectionCount`, `projectWorkerRouteSelectionWorkerCounts`, and `lastProjectWorkerRouteSelected*` mirror the selection stream, so promotion evidence should pair `/healthz.projectWorkerRoutes`, the matching `/healthz.remoteWorkers[].accountId` labels, the `gateway/projectWorkerRouteSelected` operator event, the `gateway_project_worker_route_selections{tenant_id,project_id,worker_id,account_id}` metric, and the `codex_gateway.audit` route-selection log with the `/v1/events` / health / metric / log evidence for the same tenant/project/account scope. The dedicated `thread/start` regression now also pins the audit log fields alongside the operator event and v2 health mirrors, and logs `<none>` for the account field when the route is not account-labeled. The metric uses `account_id=none` for unlabeled routes so metric samples stay valid while still distinguishing the unlabeled case from a real account id, and blank account labels are normalized into that missing-account representation before route-selection evidence is recorded. The exact deployment checklist now lives in the promotion evidence worksheet in [docs/gateway-v2-compat.md](/home/lin/project/codex/docs/gateway-v2-compat.md). `/healthz.projectWorkerRoutes` now also surfaces `accountRoutingEligible=false` when a labeled worker is unhealthy or account-exhausted, so reviewers can reject a route without having to infer that state from the separate worker fields. A dedicated remote-runtime regression now also shows one labeled worker becoming unhealthy and exhausted while another project remains eligible on its own labeled worker, keeping the mixed-eligibility state pinned at the runtime-health boundary. |
| live active-context fail-closed | `turn/start`, `turn/steer`, `turn/interrupt`, realtime append/stop, thread-scoped MCP calls, approval and elicitation replies, `review/start` | exhausted owner accounts do not silently replay live or side-effecting work on another worker; client-visible errors, `/v1/events`, `accountCapacityEvent*` health fields, exported metric counts, and audit logs identify the same worker/account/scope; the shared thread-scoped regression now pins this agreement across turn control, realtime start/append/stop, thread-scoped MCP, review start, app discovery, shell command, compact, unsubscribe, and background-terminal cleanup methods, while the real WebSocket `serverRequest/respond` regression pins the same health mirror for approval and user-input reply fail-closed behavior |
...
| notification delivery health mirror | forwarded downstream notifications and northbound notification send failures | `/healthz.v2Connections.forwardedNotificationCounts`, `lastForwardedNotificationMethod`, `lastForwardedNotificationAt`, `notificationSendFailureCounts`, `lastNotificationSendFailureMethod`, `lastNotificationSendFailureOutcome`, and `lastNotificationSendFailureAt` mirror `gateway_v2_forwarded_notifications{method}` and `gateway_v2_notification_send_failures{method,outcome}`, so rollout evidence can distinguish successful fan-in, intentional suppression, and slow-client delivery failure without relying only on metrics export or logs; the real single-worker and multi-worker slow-client health regressions now pin the notification-send failure mirror for the warning notification that triggers the terminal `client_send_timed_out` outcome |
...
| health response serialization | non-empty v2 rollout diagnostics returned by `GET /healthz` | the HTTP health route regression serializes representative non-empty values for connection outcomes, request outcomes, request rejections, fail-closed routes, upstream failures, transport pressure, routing diagnostics, notification delivery, server-request delivery failures, lifecycle events, active and terminal server-request backlog started-at / max / peak fields, and worker-specific protocol violations, so the documented fields are pinned at the northbound JSON boundary as well as in the health registry |
| background client-request observability | `command/exec` and other long-running client requests | `/healthz.v2Connections.activeConnectionPendingClientRequestStartedAt`, `activeConnectionMaxPendingClientRequestCount`, `activeConnectionPeakPendingClientRequestCount`, `activeConnectionPendingClientRequestWorkerCounts`, `activeConnectionPendingClientRequestMethodCounts`, `lastConnectionMaxPendingClientRequestCount`, `lastConnectionPendingClientRequestStartedAt`, `lastConnectionPendingClientRequestWorkerCounts`, and `lastConnectionPendingClientRequestMethodCounts` match the worker route, request method, buildup age, terminal connection outcome, lifecycle peak, and `gateway_v2_connection_pending_client_requests_by_worker` / `gateway_v2_connection_pending_client_requests_by_method` / `gateway_v2_connection_max_pending_client_requests` metrics so background request buildup remains explainable after teardown; the real slow-client `command/exec` WebSocket regression now pins the active health snapshot while the downstream request is still pending before the terminal timeout snapshot is recorded |
| server-request backlog observability | approval, user-input, elicitation, and ChatGPT token-refresh server requests | `/healthz.v2Connections.activeConnectionServerRequestBacklogWorkerCounts`, `lastConnectionServerRequestBacklogWorkerCounts`, `activeConnectionServerRequestBacklogMethodCounts`, `lastConnectionServerRequestBacklogMethodCounts`, `activeConnectionServerRequestBacklogStartedAt`, `lastConnectionServerRequestBacklogStartedAt`, `activeConnectionPeakServerRequestBacklogCount`, and `lastConnectionMaxServerRequestBacklogCount` match the worker route, prompt method family, pending versus answered-but-unresolved stage, combined backlog, buildup age, lifecycle peak, lifecycle metrics, cleanup or resolution logs, and `gateway_v2_connection_pending_server_requests_by_worker` / `gateway_v2_connection_answered_but_unresolved_server_requests_by_worker` / `gateway_v2_connection_server_request_backlog_by_worker` / `gateway_v2_connection_max_server_request_backlog` metrics; the method-family helper now has direct regression coverage for mixed pending and answered-but-unresolved user-input, permissions approval, MCP elicitation, and ChatGPT token-refresh prompts, and it groups those prompt families explicitly in the backlog method-count snapshot; the real concurrent multi-worker harness pins active and last-completed health counts plus backlog started-at and peak/max fields for those prompt classes across both workers while earlier prompt classes remain answered-but-unresolved, and the real single-worker plus multi-worker slow-client harnesses pin the same buildup-age and peak/max fields for a stranded ChatGPT token-refresh prompt before the terminal timeout snapshot; worker-loss cleanup logs also identify the resolved thread-scoped and stranded connection-scoped prompt methods |
...
| `account/chatgptAuthTokens/refresh` | TUI accepts | `transparent passthrough` | Covered by dedicated northbound server-request round-trip tests plus single-worker and multi-worker remote round-trip tests, with the real single-worker `RemoteAppServerClient` harness now covering both steady-state and post-reconnect recovery, and the multi-worker harness covering translated per-worker request IDs on one shared northbound session. Real multi-worker same-session recovery coverage now also verifies that a lazily re-added worker can resume this connection-scoped refresh round trip without leaking stale worker-local request IDs, and real multi-worker disconnect coverage now also verifies that the shared session fails closed both while a refresh prompt is still pending and while the client response is still awaiting downstream `serverRequest/resolved`; those real disconnect regressions now also verify `/v1/events` publishes `gateway/v2ServerRequestCleanup` with the stranded ChatGPT token-refresh method and downstream request id. Embedded mode currently relies on targeted northbound coverage for this path because the in-process downstream app-server client rejects ChatGPT token-refresh prompts before they can surface through the real embedded compatibility harness. |
| legacy `apply_patch` / `exec_command` approvals | TUI marks unsupported today | `transparent passthrough` | These are not part of the primary Stage A drop-in target. Dedicated northbound websocket regression coverage verifies round trips for legacy `applyPatchApproval` and `execCommandApproval`, and scope-enforcement unit coverage pins that their `conversationId` is treated as thread ownership so legacy prompts do not bypass gateway visibility checks. The real multi-worker `RemoteAppServerClient` server-request harness now also covers both legacy approval methods on one shared northbound session, including gateway-owned request-id translation across worker-owned visible threads. |
...
- thread-scoped server-request answers are also guarded by the live
  ...
  of downstream delivery, the matching
  `gateway/accountActiveThreadHandoffFailed` operator event, and the matching
  server-request lifecycle metrics.
