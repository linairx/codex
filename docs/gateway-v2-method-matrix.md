...
- client-side cleanup rejections for still-pending server requests now also
  ...
  so teardown-time cleanup failures and skips are visible through the same
  direct rejection-delivery counter
...
- synthesized worker-cleanup `serverRequest/resolved` delivery failures now
  ...
  and include tenant/project scope in the cleanup warning, so thread-scoped
  prompt dismissal failures during worker loss are tied back to the same
  notification failure telemetry as ordinary fan-in send failures
