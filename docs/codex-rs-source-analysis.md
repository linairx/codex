...
- `tui` / `exec` 的内部 CLI 字段注释现在也已继续收口到同一口径：`resume_session_target`、resume 列表开关，以及 resume 后附带 prompt/image 的字段说明都已改成显式覆盖 `resume or reconnect`，避免维护者继续从结构体注释里读出旧的纯 resume 心智
...
- `codex-app-server-client` README 的 resident continuity 说明现在也已把尾部旧措辞清干净：archived metadata repair 和 post-unsubscribe 这两处 typed 边界不再继续写成泛化的 ordinary interactive thread/session summary，而统一回到 `ordinary interactive resume target`，避免同一段 bootstrap 说明前半段已经收口、后半段又残留旧术语
- `app-server/README.md` 的 `thread/list` 详细过滤说明现在也已把 source-filter 边界自己说完整：archived 列表不再被描述成会退回普通 interactive history，而继续对齐成 ordinary interactive resume target；同时 `sourceKinds` 的字段说明也已明确写出“omit/[] 仍是 interactive-only”，想看 resident / exec / app-server / sub-agent 这类 non-interactive 线程时必须显式传对应 kinds，避免外围集成只在 debug-client / test-client README 里看见这条边界，而在主 API 文档里继续误用默认过滤
- `app-server/README.md` 的 `thread/loaded/list` / `thread/loaded/read` 详细说明现在也已把同一条 source-filter 默认语义补齐：不再只在 `thread/list` 段落单独写清 omit/`[]` 代表 interactive-only，而会明确说明 loaded probe 和 loaded summary 两条路径也继承同样默认；想看 resident / exec / app-server / sub-agent 这类 non-interactive loaded thread 时，调用方同样必须显式传对应 `sourceKinds`，避免主 API 文档在 stored 列表上说对、到了 loaded 读取面又重新留下“默认会看见全部 loaded 线程”的灰区
- `codex-tui` 的 `thread_source_kinds.rs` helper 注释现在也已把这条实现边界直接写回源码：`interactive_thread_source_kinds()` 会明确说明它保留 app-server 的 interactive-only 默认，而 `all_thread_source_kinds()` 也会明确说明自己是“主动绕过 omit/[] 默认过滤、显式包含 non-interactive 线程”的 helper，避免维护者只在 README / 测试里看见这条 source-filter 约束，却在 helper 文件本身读不出为什么需要这两个列表
- `app-server-protocol` 的 loaded 读取面参数注释现在也已纠正到真实实现：`ThreadLoadedListParams.source_kinds` 与 `ThreadLoadedReadParams.source_kinds` 不再错误写成 “empty = all source kinds”，而会明确对齐 `compute_source_filters(...)` 的实际行为，即 omit/空数组都回 interactive-only；这一步把协议源码注释、主 README 和实际过滤逻辑重新锁回同一口径，避免维护者在最底层类型定义里再次读到错误默认值
- `codex-app-server-client` README 的 rollback continuity 说明现在也已把最后一处泛化尾词清掉：resident assistant 在 `thread/rollback` 响应面上的 typed 连续性不再被描述成会退回 generic interactive history summary，而继续统一成 `ordinary interactive resume target summary`，避免同一段 resident continuity 文档前半段已经完全收口、却在 rollback 这一句重新滑回旧心智
- `codex-app-server-client` README 的 rollback continuity 句子现在也已顺手磨平到可直接复用的措辞：它不再停在语义虽然正确、但读起来别扭的 “generic ordinary interactive ...” 过渡句，而会直接写成 “reconstructing the rollout as an ordinary interactive resume target summary”，避免外围维护者在复制这类 resident continuity 说明时再次引入不自然的半收口英文
...
- 顶层 `codex fork` 现在也已补上与 `resume` 对称的 source-filter 控制：`--include-non-interactive` 会继续贯穿 `fork --last`、显式 lookup 与 picker 初始过滤，而 fork picker 自己也能直接按 `i` 在 interactive-only 与全量 source kinds 间切换，避免 resident / exec 这类 non-interactive 线程只在 reconnect 路径可见、到了 fork 路径又被默认过滤静默漏掉
- 顶层 `codex --help` / `codex fork --help` 的 fork 摘要现在也已继续跟上这条能力边界：用户可见简介不再把 fork 写死成“interactive-only”入口，而会稳定对齐到“previous session”与“most recent recorded session”这类覆盖 non-interactive 线程的表述，避免实现已支持 `--include-non-interactive`，但最靠前帮助文案仍把用户心智锁回旧的 interactive-only 路径
- `codex-rs/README.md` 现在也已把这层 session-management 能力写回最靠前入口：README 不再只讲 `codex exec` 的 resident reconnect 摘要，而会直接补上 `codex resume` / `codex fork` 的 picker、`--last` 与 `--include-non-interactive` 用法，避免 help 与实现都已支持 non-interactive resident 选择，但仓库首页说明仍让用户以为这只是内部或次级入口能力
- `docs/getting-started.md` 这个更靠前的新手入口现在也已不再只是一个外链占位：它会直接给出 `codex` / `codex resume` / `codex fork` / `--last` / `--include-non-interactive` 的最小起步路径，并明确 resident assistant 仍通过 `codex resume` 进入，只是在 `thread.mode = residentAssistant` 时产品动作会表述成 reconnect，避免首页与 README 都已收口后，真正 first-run 文档入口仍完全漏掉这条 session-management 主线
- `docs/slash_commands.md` 与 `docs/exec.md` 这两个一级入口文档现在也已不再只是外链壳：前者会直接写明 `/resume` 既是 saved-session picker 也是 resident assistant reconnect 入口，后者则会直接给出 `codex exec` / `codex exec resume` / `--last` / `--json` 的最小 bootstrap 说明，避免更上层 README 和 help 都已 resident-aware，但这两份专门入口文档仍完全不承接本地实现现状
- 仓库根 [README.md](/home/lin/project/codex/README.md) 现在也已把这条 session-management 主线补到真正最外层入口：不再只停留在“运行 `codex` 开始”，而会直接列出 `codex resume` / `codex fork` / `--last` / `SESSION_ID_OR_NAME` / `--include-non-interactive` 这些最常见流转，避免 `codex-rs/README.md` 和 docs 层都已收口后，仓库首页仍让第一次进入仓库的读者完全看不到 saved-session / reconnect / fork 主线
- 仓库根 [README.md](/home/lin/project/codex/README.md) 的 Docs 列表与 [docs/install.md](/home/lin/project/codex/docs/install.md) 现在也已继续承接这条入口链：前者会把 `Getting started` 直接挂进首页文档导航，后者则会在 build/run 示例后直接指回 `getting-started` 和 `codex-rs/README.md` 的 session-management 说明，避免首页正文虽然已提到 `resume/fork`，但文档导航和安装页仍把 first-run 用户丢回零散入口
- `codex fork` 的显式目标说明现在也已继续和实现对齐：虽然底层 lookup 早就支持“UUID 优先、否则按线程名查找”，但 fork 的 CLI/help/README/getting-started 之前仍把它写成 id-only；这层文案现在已统一改成 `SESSION_ID_OR_NAME`，而且 named non-interactive fork target 失败时也会像 resume 一样提示 `--include-non-interactive`，避免实现支持按名 fork，但最前面的提示和缺失目标报错还把用户误导回 UUID-only 心智
- `codex-tui` 的缺失目标提示现在也已把名字语义自己说对：既然 `resume`/`fork` 两条路径都支持“UUID 优先、否则按线程名查找”，fatal 提示不再继续写成 “No saved session found with ID ...”，而会统一改成 “ID or name ...”；对应回归同时覆盖 resume / fork / UUID 这三类分支，避免名字查找已落地很久，但最终报错文案仍把用户重新带回 ID-only 心智
- 顶层 `codex` / `codex exec` 的 positional 参数名和 TUI 内部桥接命名现在也已继续收口：用户可见 help 不再一边正文写“UUID or thread name”、一边 positional 仍显示 `SESSION_ID` 或 “by id”，而会统一改成 `SESSION_ID_OR_NAME` / `by id or name`；同时 `codex-tui` 内部桥接字段也已从 `resume_session_id` / `fork_session_id` 进一步改成 `resume_session_target` / `fork_session_target`，避免外层 CLI 语义已支持按名恢复/分叉，但内部状态名和维护注释仍把阅读者带回 UUID-only 心智
- `codex-tui` 的缺失目标提示现在也已把“空参数回 picker”这半句一并说对：既然 `resume`/`fork` 的直达目标已经从 UUID-only 扩成 `ID or name`，fatal 提示里的后续引导也不再继续写成 “without an ID”，而会统一改成 “without a target”；对应断言同时覆盖 resume / fork 两条路径，避免第一句已经承认 name lookup，第二句却又把用户引导回 ID-only 心智
